...
clear
echo "======================================================"
echo "   SoCal IT - ThinkPad BIOS Patch Tool (xx40) v1.0   "
echo "======================================================"
...
EOF
  ...
  echo " - UNPLUG CH341A USB before touching clip"
  echo "======================================================"
  read -p "Attach clip, then plug in CH341A. Press Enter to continue..."

  echo "[*] Dumping BIOS..."
  sudo flashrom -p ch341a_spi -r bios1.img
  sudo flashrom -p ch341a_spi -r bios2.img

  echo "[*] Verifying BIOS dump..."
  ...
  sudo flashrom -p ch341a_spi -w "$PATCHED_BIOS"
...
if [[ "$CHOICE" == "2" ]]; then
  ...
  sudo flashrom -p ch341a_spi -w "$BACKUP_BIOS"
